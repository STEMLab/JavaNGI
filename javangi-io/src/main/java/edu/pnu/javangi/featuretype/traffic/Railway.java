...
public class Railway extends AbstractFeatureType{       // 철도 A0150000
	...
	public Railway(){
		...
		metaDataTableName = "railwaymetadata";
		dataTableName = "railwaydata";
	}
	
	public Railway(int layerID, String layerName){
	...
	public String[] getInsertQuery(String fileName) {
		...
		for(int i=0; i<recordID.size(); i++){
			query = "insert into " + dataTableName + " values('" + fileName + "', " + layerID + ", " + recordID.get(i) + ", '"
					+ name.get(i) + "', '" + classification.get(i) + "', '" + managementAgency.get(i) + "', '" + etc.get(i) + "', '" + UFID.get(i) + "', "
	...
	public String[] getCreateSchema() {
		...
		schema[1] = "CREATE TABLE IF NOT EXISTS " + dataTableName + "(  "
				...
				+ "  name text,  classification text, managementAgency text, etc text, "
	...
	public void exportDataToFile(PrintWriter ngiWriter, PrintWriter ndaWriter) {
		...
		for(int i=0; i<recordID.size(); i++){
			...
			ndaWriter.println(recordID.get(i) + ", "
					+ name.get(i) + ", " + classification.get(i) + ", " + managementAgency.get(i) + ", " + etc.get(i) + ", "
					+ UFID.get(i));
		}
