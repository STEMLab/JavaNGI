...
public class Crossroad extends AbstractFeatureType{     // 교차로 A0080000
	...
	public Crossroad(){
		...
		metaDataTableName = "crossroadmetadata";
		dataTableName = "crossroaddata";
	}
	
	public Crossroad(int layerID, String layerName){
	...
	public String[] getInsertQuery(String fileName) {
		...
		for(int i=0; i<recordID.size(); i++){
			query = "insert into " + dataTableName + " values('" + fileName + "', " + layerID + ", " + recordID.get(i) + ", '"
					+ name.get(i) + "', '" + type.get(i) + "', '" + UFID.get(i) + "', " + wkts.get(i) + ", '" + gattrs.get(i) + "')";
