...
public class AmusementFacility extends AbstractFeatureType{     // 놀이시설 C0380000
	...
	public AmusementFacility(){
		...
		metaDataTableName = "amusementfacilitymetadata";
		dataTableName = "amusementfacilitydata";
	}
	
	public AmusementFacility(int layerID, String layerName){
	...
	public String[] getInsertQuery(String fileName) {
		...
		for(int i=0; i<recordID.size(); i++){
			query = "insert into " + dataTableName + " values('" + fileName + "', " + layerID + ", " + recordID.get(i) + ", '"
					+ name.get(i) + "', '" + classification.get(i) + "', '" + UFID.get(i) + "', " + wkts.get(i) + ", '" + gattrs.get(i) + "')";
	...
	public void exportDataToFile(PrintWriter ngiWriter, PrintWriter ndaWriter) {
		...
		for(int i=0; i<recordID.size(); i++){
			...
			ndaWriter.println(recordID.get(i) + ", "
					+ name.get(i) + ", " + classification.get(i) + ", "
					+ UFID.get(i));
		}
