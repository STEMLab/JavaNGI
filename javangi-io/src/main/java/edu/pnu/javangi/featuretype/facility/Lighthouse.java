...
public class Lighthouse extends AbstractFeatureType{    // 등대 C0160000
	...
	public Lighthouse(){
		...
		metaDataTableName = "lighthousemetadata";
		dataTableName = "lighthousedata";
	}
	
	public Lighthouse(int layerID, String layerName){
	...
	public String[] getInsertQuery(String fileName) {
		...
		for(int i=0; i<recordID.size(); i++){
			query = "insert into " + dataTableName + " values('" + fileName + "', " + layerID + ", " + recordID.get(i) + ", '"
					+ name.get(i) + "', '" + purpose.get(i) + "', '" + form.get(i) + "', '" + managementAgency.get(i) + "', '"
	...
	public String[] getCreateSchema() {
		...
		schema[1] = "CREATE TABLE IF NOT EXISTS " + dataTableName + "(  "
				...
				+ "  name text,  purpose text, form text, managementagency text, "
