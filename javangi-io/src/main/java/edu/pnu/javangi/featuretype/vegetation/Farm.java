...
public class Farm extends AbstractFeatureType{ // ����	D0040000
	...
	public Farm(){
		...
		metaDataTableName = "farmmetadata";
		dataTableName = "farmdata";
	}
	
	public Farm(int layerID, String layerName){
	...
	public String[] getInsertQuery(String fileName) {
		...
		for(int i=0; i<recordID.size(); i++){
			query = "insert into " + dataTableName + " values('" + fileName + "', " + layerID + ", " + recordID.get(i) + ", '"
					+ name.get(i) + "', '" + purpose.get(i) + "', '" + etc.get(i) + "', '"
	...
	public String[] getCreateSchema() {
		...
		schema[1] = "CREATE TABLE IF NOT EXISTS " + dataTableName + "(  "
				...
				+ "name text, purpose text, etc text, "
