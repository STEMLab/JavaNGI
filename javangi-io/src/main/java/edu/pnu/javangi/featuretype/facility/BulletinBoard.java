...
public class BulletinBoard extends AbstractFeatureType{ // �Խ��� C0400000
	...
	public BulletinBoard(){
		...
		metaDataTableName = "bulletinboardmetadata";
		dataTableName = "bulletinboarddata";
	}
	
	public BulletinBoard(int layerID, String layerName){
	...
	public String[] getInsertQuery(String fileName) {
		...
		for(int i=0; i<recordID.size(); i++){
			query = "insert into " + dataTableName + " values('" + fileName + "', " + layerID + ", " + recordID.get(i) + ", '"
					+ classification.get(i) + "', '" + UFID.get(i) + "', " + wkts.get(i) + ", '" + gattrs.get(i) + "')";
