...
public class Observatory extends AbstractFeatureType{   // 관측소 C0260000
	...
	public Observatory(){
		...
		metaDataTableName = "observatorymetadata";
		dataTableName = "observatorydata";
	}
	
	public Observatory(int layerID, String layerName){
	...
	public String[] getInsertQuery(String fileName) {
		...
		for(int i=0; i<recordID.size(); i++){
			query = "insert into " + dataTableName + " values('" + fileName + "', " + layerID + ", " + recordID.get(i) + ", '"
					+ name.get(i) + "', '" + purpose.get(i) + "', '" + UFID.get(i) + "', " + wkts.get(i) + ", '" + gattrs.get(i) + "')";
