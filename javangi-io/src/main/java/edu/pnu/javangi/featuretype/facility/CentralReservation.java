...
public class CentralReservation extends AbstractFeatureType{    // 도로분리대 C0520000
	...
	public CentralReservation(){
		...
		metaDataTableName = "centralreservationmetadata";
		dataTableName = "centralreservationdata";
	}
	
	public CentralReservation(int layerID, String layerName){
	...
	public void parsingNDAData(String recordNum, String recordData){
		...
		structure.add(recordDataSplit[1]);
	...
	public String[] getInsertQuery(String fileName) {
		...
		for(int i=0; i<recordID.size(); i++){
			query = "insert into " + dataTableName + " values('" + fileName + "', " + layerID + ", " + recordID.get(i) + ", '"
					+ structure.get(i) + "', '" + UFID.get(i) + "', " + wkts.get(i) + ", '" + gattrs.get(i) + "')";
