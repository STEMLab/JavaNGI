...
public class RailwayCenterLine extends AbstractFeatureType {    // 철도중심선 A0171119
	...
	public RailwayCenterLine(){
		...
		metaDataTableName = "railwaycenterlinemetadata";
		dataTableName = "railwaycenterlinedata";
	}
	
	public RailwayCenterLine(int layerID, String layerName){
	...
	public String getStructureN(int n){
	...
	public void parsingNDAData(String recordNum, String recordData){
		...
		structure.add(recordDataSplit[3]);
		managementAgency.add(recordDataSplit[4]);
		etc.add(recordDataSplit[5]);
	...
	public String[] getInsertQuery(String fileName) {
		...
		for(int i=0; i<recordID.size(); i++){
			query = "insert into " + dataTableName + " values('" + fileName + "', " + layerID + ", " + recordID.get(i) + ", '"
					+ name.get(i) + "', '" + classification.get(i) + "', '" + structure.get(i) + "', '" + managementAgency.get(i) + "', '"
	...
	public String[] getCreateSchema() {
		...
		schema[1] = "CREATE TABLE IF NOT EXISTS " + dataTableName + "(  "
				...
				+ "  name text,  classification text, structure text, managementAgency text, etc text, "
	...
	public void exportDataToFile(PrintWriter ngiWriter, PrintWriter ndaWriter) {
		...
		for(int i=0; i<recordID.size(); i++){
			...
			ndaWriter.println(recordID.get(i) + ", "
					+ name.get(i) + ", " + classification.get(i) + ", " + structure.get(i) + ", "
					+ managementAgency.get(i) + ", " + etc.get(i) + ", "
					+ UFID.get(i));
		}
		...
		if(tableType.equals("Data")){
			try{
				...
				while(rs.next()){
					...
					String _structure = rs.getString("structure");
