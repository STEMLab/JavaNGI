...
public class Yard extends AbstractFeatureType{          // 적치장 C0200000
	...
	public Yard(){
		...
		metaDataTableName = "yardmetadata";
		dataTableName = "yarddata";
	}
	
	public Yard(int layerID, String layerName){
	...
	public String[] getInsertQuery(String fileName) {
		...
		for(int i=0; i<recordID.size(); i++){
			query = "insert into " + dataTableName + " values('" + fileName + "', " + layerID + ", " + recordID.get(i) + ", '"
					+ name.get(i) + "', '" + purpose.get(i) + "', '" + managementAgency.get(i) + "', '"
	...
	public String[] getCreateSchema() {
		...
		schema[1] = "CREATE TABLE IF NOT EXISTS " + dataTableName + "(  "
				...
				+ "  name text,  purpose text, managementagency text, "
