...
public class ContourLine extends AbstractFeatureType{   // 등고선 F0010000
	...
	public ContourLine(){
		...
		metaDataTableName = "contourlinemetadata";
		dataTableName = "contourlinedata";
	}
	
	public ContourLine(int layerID, String layerName){
	...
	public void parsingNDAData(String recordNum, String recordData){
		...
		value.add(Double.valueOf(recordDataSplit[2]));
	...
	public String[] getInsertQuery(String fileName) {
		...
		for(int i=0; i<recordID.size(); i++){
			query = "insert into " + dataTableName + " values('" + fileName + "', " + layerID + ", " + recordID.get(i) + ", '"
					+ classification.get(i) + "', " + value.get(i) + ", '"
	...
	public String[] getCreateSchema() {
		...
		schema[1] = "CREATE TABLE IF NOT EXISTS " + dataTableName + "(  "
				...
				+ "classification text, value double precision, "
		...
		if(tableType.equals("Data")){
			try{
				...
				while(rs.next()){
					...
					Double _value = rs.getDouble("value");
