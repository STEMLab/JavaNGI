...
public class Stone extends AbstractFeatureType{ // 비석/기념비 C0330000
	...
	public Stone(){
		...
		metaDataTableName = "stonemetadata";
		dataTableName = "stonedata";
	}
	
	public Stone(int layerID, String layerName){
	...
	public void parsingNDAData(String recordNum, String recordData){
		...
		height.add(Double.valueOf(recordDataSplit[3]));
	...
	public String[] getInsertQuery(String fileName) {
		...
		for(int i=0; i<recordID.size(); i++){
			query = "insert into " + dataTableName + " values('" + fileName + "', " + layerID + ", " + recordID.get(i) + ", '"
					+ name.get(i) + "', '" + classification.get(i) + "', " + height.get(i) + ", '" + etc.get(i) + "', '"
	...
	public String[] getCreateSchema() {
		...
		schema[1] = "CREATE TABLE IF NOT EXISTS " + dataTableName + "(  "
				...
				+ "  name text,  classification text, height double precision, etc text, "
