...
public class Underpass extends AbstractFeatureType{ // ���ϵ� C0453322
	...
	public Underpass(){
		...
		metaDataTableName = "underpassmetadata";
		dataTableName = "underpassdata";
	}
	
	public Underpass(int layerID, String layerName){
	...
	public String[] getCreateSchema() {
		...
		schema[1] = "CREATE TABLE IF NOT EXISTS " + dataTableName + "(  "
				...
				+ "  name text,  extension double precision, width double precision, height double precision, etc text, "
	...
	public void exportDataToFile(PrintWriter ngiWriter, PrintWriter ndaWriter) {
		...
		for(int i=0; i<recordID.size(); i++){
			...
			ndaWriter.println(recordID.get(i) + ", "
					+ name.get(i) + ", " + extension.get(i) + ", "	+ width.get(i) + ", " + height.get(i) + ", " + etc.get(i) + ", "
					+ UFID.get(i));
		}
