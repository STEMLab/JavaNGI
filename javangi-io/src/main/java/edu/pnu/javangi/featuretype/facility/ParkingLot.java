...
public class ParkingLot extends AbstractFeatureType{ // ������ C0430000
	...
	public ParkingLot(){
		...
		metaDataTableName = "parkinglotmetadata";
		dataTableName = "parkinglotdata";
	}
	
	public ParkingLot(int layerID, String layerName){
	...
	public Double getCapacityN(int n){
		return capacity.get(n);
	}
	...
	public void parsingNDAData(String recordNum, String recordData){
		...
		capacity.add(Double.valueOf(recordDataSplit[3]));
	...
	public String[] getInsertQuery(String fileName) {
		...
		for(int i=0; i<recordID.size(); i++){
			query = "insert into " + dataTableName + " values('" + fileName + "', " + layerID + ", " + recordID.get(i) + ", '"
					+ name.get(i) + "', '" + classification.get(i) + "', " + capacity.get(i) + ", '"
	...
	public String[] getCreateSchema() {
		...
		schema[1] = "CREATE TABLE IF NOT EXISTS " + dataTableName + "(  "
				...
				+ "  name text,  classification text, capacity double precision, "
		...
		if(tableType.equals("Data")){
			try{
				...
				while(rs.next()){
					...
					double _capacity = rs.getDouble("capacity");
