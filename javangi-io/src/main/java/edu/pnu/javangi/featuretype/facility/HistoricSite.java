...
public class HistoricSite extends AbstractFeatureType{ // ������ C0305316
	...
	public HistoricSite(){
		...
		metaDataTableName = "historicsitemetadata";
		dataTableName = "historicsitedata";
	}
	
	public HistoricSite(int layerID, String layerName){
	...
	public String[] getInsertQuery(String fileName) {
		...
		for(int i=0; i<recordID.size(); i++){
			query = "insert into " + dataTableName + " values('" + fileName + "', " + layerID + ", " + recordID.get(i) + ", '"
					+ name.get(i) + "', '" + etc.get(i) + "', '" + UFID.get(i) + "', " + wkts.get(i) + ", '" + gattrs.get(i) + "')";
