...
public class CulturalAssets extends AbstractFeatureType{        // 문화재 C0310000
	private ArrayList<String> name;                        // 명칭
	private ArrayList<String> form;                        // 형태
		
	public CulturalAssets(){
		super();
		layerName = "C0310000";
		name = new ArrayList<String>();
		form = new ArrayList<String>();
		
		metaDataTableName = "culturalassetsmetadata";
		dataTableName = "culturalassetsdata";
	}
	
	public CulturalAssets(int layerID, String layerName){
	...
	public String[] getInsertQuery(String fileName) {
		...
		for(int i=0; i<recordID.size(); i++){
			query = "insert into " + dataTableName + " values('" + fileName + "', " + layerID + ", " + recordID.get(i) + ", '"
					+ name.get(i) + "', '" + form.get(i) + "', '" + UFID.get(i) + "', " + wkts.get(i) + ", '" + gattrs.get(i) + "')";
