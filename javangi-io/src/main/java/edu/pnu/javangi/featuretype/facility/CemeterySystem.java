...
public class CemeterySystem extends AbstractFeatureType{ // 묘지계 C0290000
	...
	public CemeterySystem(){
		...
		metaDataTableName = "cemeterysystemmetadata";
		dataTableName = "cemeterysystemdata";
	}
	
	public CemeterySystem(int layerID, String layerName){
	...
	public String[] getInsertQuery(String fileName) {
		...
		for(int i=0; i<recordID.size(); i++){
			query = "insert into " + dataTableName + " values('" + fileName + "', " + layerID + ", " + recordID.get(i) + ", '"
					+ name.get(i) + "', '" + managementAgency.get(i) + "', '" + UFID.get(i) + "', " + wkts.get(i) + ", '" + gattrs.get(i) + "')";
	...
	public String[] getCreateSchema() {
		...
		schema[1] = "CREATE TABLE IF NOT EXISTS " + dataTableName + "(  "
				...
				+ "  name text, managementAgency text, "
	...
	public void exportDataToFile(PrintWriter ngiWriter, PrintWriter ndaWriter) {
		...
		for(int i=0; i<recordID.size(); i++){
			...
			ndaWriter.println(recordID.get(i) + ", "
					+ name.get(i) + ", " + managementAgency.get(i) + ", "
					+ UFID.get(i));
		}
		...
		if(tableType.equals("Data")){
			try{
				...
				while(rs.next()){
					...
					String _managementeAgency = rs.getString("managementagency");
					
					name.add(_name);
					managementAgency.add(_managementeAgency);
				}
