...
public class Spa extends AbstractFeatureType{   // 온천 C0125335
	...
	public Spa(){
		...
		metaDataTableName = "spametadata";
		dataTableName = "spadata";
	}
	
	public Spa(int layerID, String layerName){
	...
	public String getIngredientN(int n){
		return ingredient.get(n);
	}
	...
	public void parsingNDAData(String recordNum, String recordData){
		...
		ingredient.add(recordDataSplit[2]);
	...
	public String[] getInsertQuery(String fileName) {
		...
		for(int i=0; i<recordID.size(); i++){
			query = "insert into " + dataTableName + " values('" + fileName + "', " + layerID + ", " + recordID.get(i) + ", '"
					+ name.get(i) + "', '" + ingredient.get(i) + "', '" + etc.get(i) + "', '"
	...
	public String[] getCreateSchema() {
		...
		schema[1] = "CREATE TABLE IF NOT EXISTS " + dataTableName + "(  "
				...
				+ "  name text,  ingredient text, etc text, "
		...
		if(tableType.equals("Data")){
			try{
				...
				while(rs.next()){
					...
					String _ingredient = rs.getString("ingredient");
