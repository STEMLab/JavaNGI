...
public class PlaceName extends AbstractFeatureType{     // 지명 H0040000
	...
	public PlaceName(){
		...
		metaDataTableName = "placenamemetadata";
		dataTableName = "placenamedata";
	}
	
	public PlaceName(int layerID, String layerName){
	...
	public String[] getInsertQuery(String fileName) {
		...
		for(int i=0; i<recordID.size(); i++){
			query = "insert into " + dataTableName + " values('" + fileName + "', " + layerID + ", " + recordID.get(i) + ", '"
					+ name.get(i) + "', '" + classification.get(i) + "', '" + form.get(i) + "', '"
					+ UFID.get(i) + "', " + wkts.get(i) + ", '" + point_text.get(i) + "', '" + gattrs.get(i) + "')";
	...
	public String[] getCreateSchema() {
		...
		schema[1] = "CREATE TABLE IF NOT EXISTS " + dataTableName + "(  "
				...
				+ "name text, classification text, form text, "
				+ "ufid text,  shape geometry, pointtext text, gattr text, "
