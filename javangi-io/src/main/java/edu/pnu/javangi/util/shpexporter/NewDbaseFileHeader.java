 ...
 * @source $URL$
 */
public class NewDbaseFileHeader extends DbaseFileHeader {
         ...
         * 
         * Calendar refCal = (Calendar) new GregorianCalendar(TimeZone.getTimeZone("UTC")); refCal.set(Calendar.ERA, GregorianCalendar.BC);
         * refCal.set(Calendar.YEAR, 4713); refCal.set(Calendar.MONTH, Calendar.JANUARY); refCal.set(Calendar.DAY_OF_MONTH, 1);
         * refCal.set(Calendar.HOUR, 12); refCal.set(Calendar.MINUTE, 0); refCal.set(Calendar.SECOND, 0); refCal.set(Calendar.MILLISECOND, 0);
         * MILLIS_SINCE_4713 = refCal.getTimeInMillis() - 43200000L; //(43200000L: 12 hour correction factor taken from DBFViewer2000)
         */
        ...
        /**
         * Add a column to this DbaseFileHeader. The type is one of (C N L or D) character, number, logical(true/false), or date. The Field length is
         * the total length in bytes reserved for this column. The decimal count only applies to numbers(N), and floating point values (F), and refers
         * to the number of characters to reserve after the decimal point. <B>Don't expect miracles from this...</B>
         * 
         * <PRE>
         ...
         * 
         * @param channel A readable byte channel. If you have an InputStream you need to use, you can call java.nio.Channels.getChannel(InputStream
         *        in).
        ...
        public void writeHeader(WritableByteChannel out) throws IOException {
                ...
                try {
                        ...
                        for (int i = 0; i < fields.length; i++) {
                                ...
                                /*
                                if(isKorean(fields[i].fieldName)) {
                                        int length = fields[i].fieldName.length();
                                        buffer.wrap(fields[i].fieldName.getBytes("EUC-KR"));
                                        for(int j = 0; j < 11 - 2 * length; j++) {
                                                buffer.put((byte) 0);
                                        }
                                }else {
                                */       
